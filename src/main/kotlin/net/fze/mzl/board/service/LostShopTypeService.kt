...
import net.fze.mzl.board.repo.LostShopTypeJpaRepository
...
/** 流失店铺类型表服务  */
@ApplicationScoped
class LostShopTypeService {
    @Inject@field:Default
    lateinit var repo: LostShopTypeJpaRepository

    fun parseId(id:Any):Long{return TypesConv.toLong(id)}

    // 根据ID查找流失店铺类型表
    fun findByIdOrNull(id:Int):LostShopTypeEntity?{
        return this.repo.findById(this.parseId(id))
    }

    // 保存流失店铺类型表
    fun saveLostShopType(e: LostShopTypeEntity):Error? {
        return catch {
            var dst: LostShopTypeEntity
            if (e.id > 0) {
                dst = this.repo.findById(this.parseId(e.id))!!
            } else {
                dst = LostShopTypeEntity()
            }
    ...
    // 批量保存流失店铺类型表
    fun saveAllLostShopType(entities:Iterable<LostShopTypeEntity>){
